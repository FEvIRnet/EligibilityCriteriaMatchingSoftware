const FOI33460check3 = (bundledResourcesArray) => {
  let descriptor = "check if BMI >= 35 kg/m2";
    ...
    if (resource.status === "final" || resource.status === "amended" || resource.status === "corrected") {
      if (resource.valueQuantity) {
        if (resource.valueQuantity.code === "kg/m2" || resource.valueQuantity.unit === "kg/m2") {
          if (resource.valueQuantity.comparator === "<" && resource.valueQuantity.value <= 35) {
